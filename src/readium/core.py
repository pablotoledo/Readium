...
from .config import DEFAULT_EXCLUDE_DIRS, DEFAULT_EXCLUDE_FILES, DEFAULT_INCLUDE_EXTENSIONS
...
class Readium:
    ...
    def is_binary(self, file_path: Union[str, Path]) -> bool:
        ...
        try:
            with open(file_path, 'rb') as file:
                ...
                return bool(chunk.translate(None, bytes([7, 8, 9, 10, 12, 13, 27] + list(range(0x20, 0x100)))))
        except Exception:
            return True
    ...
    def should_process_file(self, file_path: Union[str, Path]) -> bool:
        ...
        # Check extension
        if not any(str(file_path).lower().endswith(ext) for ext in self.config.include_extensions):
            return False
    ...
    def read_docs(self, path: Union[str, Path]) -> Tuple[str, str, str]:
        ...
        if self.config.target_dir:
            ...
            if not base_path.exists():
                raise ValueError(f"Target directory not found: {self.config.target_dir}")
            path = base_path
